 ...
 * the word, so the wording stays in the locale files; `icon` names an entry of
 * `weather_pixels`. */
...
/* The weather icons are drawn on a 16 by 16 grid, one letter per pixel, in the
 * sixteen-colour Windows palette, so they look the same on every device and
 * can be edited as text. `.` is transparent. CSS scales each one by a whole
 * number (see `app.css`), which keeps the pixels crisp. */
const weather_palette = {
  K: "#000000", // outline
  Y: "#ffff00", // sun
  O: "#808000", // sun rays
  W: "#ffffff", // cloud light
  S: "#c0c0c0", // cloud
  G: "#808080", // cloud shade, fog
  B: "#0000ff", // rain, snow
};

const weather_pixels = {
  clear: `
    .......OO.......
    .......OO.......
    ..O..........O..
...
const weather_svgs = new Map();

/* The icon as inline SVG markup, or an empty string for a group with none.
 * Built once per icon: the markup comes only from the constants above, never
 * from the forecast, so it is safe to insert as HTML. Runs of one colour on a
 * row share a single rect. */
function weather_icon(name) {
  if (!name) return "";
  if (weather_svgs.has(name)) return weather_svgs.get(name);
  ...
  weather_pixels[name]
    ...
    .forEach((row, y) => {
      for (let x = 0; x < row.length; ) {
        ...
        if (color !== ".") rects += `<rect x="${x}" y="${y}" width="${end - x}" height="1" fill="${weather_palette[color]}"/>`;
        x = end;
      }
    });
  const svg = `<svg viewBox="0 0 16 16" shape-rendering="crispEdges" aria-hidden="true" focusable="false">${rects}</svg>`;
  weather_svgs.set(name, svg);
  return svg;
}
