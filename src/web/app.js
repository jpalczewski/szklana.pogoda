 ...
 * word for. Each group names the `data-wmo-*` attribute that carries its label,
 * so the wording stays in the locale files. */
function weather_group(code) {
  if (code === 0) return { label: "wmoClear", icon: "☀️" };
  if (code === 1 || code === 2) return { label: "wmoPartly", icon: "⛅" };
  if (code === 3) return { label: "wmoCloudy", icon: "☁️" };
  if (code === 45 || code === 48) return { label: "wmoFog", icon: "🌫️" };
  if (code >= 51 && code <= 57) return { label: "wmoDrizzle", icon: "🌦️" };
  if (code >= 61 && code <= 67) return { label: "wmoRain", icon: "🌧️" };
  if ((code >= 71 && code <= 77) || code === 85 || code === 86) return { label: "wmoSnow", icon: "🌨️" };
  if (code >= 80 && code <= 82) return { label: "wmoShowers", icon: "🌦️" };
  if (code >= 95 && code <= 99) return { label: "wmoThunderstorm", icon: "⛈️" };
...
function forecast() {
  return {
    ...
    describe(code) {
      ...
      return { icon: group.icon, text: group.label ? document.body.dataset[group.label] : "—" };
    },
